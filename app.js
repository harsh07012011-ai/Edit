...
function isVideoStyle(key) {
  return STYLE_PRESETS[key]?.needsVideo === true;
}
...
function renderStyleGrid() {
  ...
  Object.values(STYLE_PRESETS).forEach((s) => {
    ...
    card.addEventListener('click', () => {
      ...
      const isVideo = isVideoStyle(s.key);
      ...
      el('video-drop').style.display = isVideo ? 'flex' : 'none';
      el('image-drop').style.display = isVideo ? 'none' : 'flex';
...
function updateGenerateButton() {
  const mediaReady = isVideoStyle(state.styleKey) ? !!state.videoFile : state.imageFiles.length > 0;
  const hasBasics = state.styleKey && mediaReady && state.audioFile;
...
function wireUploads() {
  ...
  el('video-input').addEventListener('change', (e) => {
    ...
    el('video-filename').textContent = state.videoFile ? state.videoFile.name : '';
    el('video-drop').classList.toggle('filled', !!state.videoFile);
    updateGenerateButton();
  });
...
async function generateVideo() {
  ...
  try {
    ...
    if (isVideoStyle(state.styleKey)) {
      // Single-video styles bypass the image pipeline entirely and drive
      // playback straight off the uploaded video element.
      el('progress-label').textContent = 'Loading video…';
      ...
      template = new TemplateCls(videoEl, analyzer);
    } else {
...
function resetToStart() {
  ...
  el('video-drop').style.display = 'none';
  el('video-drop').classList.remove('filled');
  el('image-drop').style.display = 'flex';
  updateGenerateButton();
  showPanel(null);
}
